...
import { setCombinedSpecies } from '@/store/sheetSlice';
...
const useCombinedSpecies = () => {
  ...
  useEffect(() => {
    ...
    if (!subSpecies) {
      dispatch(setCombinedSpecies(species));
      return;
    }
    if (!species) {
      dispatch(setCombinedSpecies(subSpecies));
      return;
    }
    ...
    dispatch(setCombinedSpecies(result));
  }, [character, dispatch]);
};
