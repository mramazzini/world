import { getItemsByToolGroup } from '@/lib/actions/db/item/read.actions';
...
const item_grouped_DATA_KEY = 'itemToolGroupData';

const useQueryItemToolGroup = (group: ToolGroup) => {
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<ItemInfo[]>([]);

  const refetch = useCallback(async () => {
    setLoading(true);
    const data = await getItemsByToolGroup(group);
    setItems(data);
    setLoading(false);
...
export default useQueryItemToolGroup;
