...
const ClassPDF = () => {
  ...
  return classData ? (
    <PDFViewer className="w-screen h-screen p-4">
      <Document>
        ...
        <Page size="A4" style={{ padding: 40 }}>
          ...
          <View>
            ...
            <Text style={{ fontSize: 12, marginBottom: 2 }}>
              Saving Throws:{" "}
              {classData.savingThrows.map((s) => AbilityToText(s)).join(", ") ||
                "None"}
            </Text>
          </View>
          <View>
                  ...
                  <View key={level}>
                    {classData.abilityScoreLevels[0] === level && (
                      <View>
                        ...
                        <Text style={{ fontSize: 10, marginBottom: 10 }}>
                          When you reach{" "}
                          {numPlace(classData.abilityScoreLevels[0])} level, and
                          again at{" "}
                          {[...classData.abilityScoreLevels] // make copy since we dont want to mutate the original
                            .map((lvl, index) => {
                              ...
                              if (
                                ...
                                return (
                                  <Text key={index}>and {numPlace(lvl)} </Text>
                                );
                              return <Text key={index}>{numPlace(lvl)}, </Text>;
                            })}{" "}
                    ...
                    {classData.subfeatLevels[0] === level && (
                      <View>
                        ...
                        <Text style={{ fontSize: 10, marginBottom: 10 }}>
                          When you reach {numPlace(classData.subfeatLevels[0])}{" "}
                          level, {classData.subClassDesc}
                        </Text>
                        <Text style={{ fontSize: 10, marginBottom: 10 }}>
