    ...
    <main className={`p-4 md:p-8 ${loading && 'cursor-wait'}`}>
      <div className="flex flex-col md:flex-row justify-between">
        <div className="flex flex-col md:w-4/5">
          <h1>Spell List: {spellList.name.toCapitalCase() || 'Class Name'}</h1>
          <p className="italic pr-4">
            <NewLineParse>
              {spellList.description || 'Your Class Description will go here.'}
            </NewLineParse>
          </p>
        </div>
              ...
              <table className="table w-full table-zebra overflow-hidden table-md">
                <thead>
                  <tr className="bg-black/30">
                    <th className="w-[20%]">Name</th>
                    <th className="w-[20%]">Range</th>
                    <th className="w-[20%] hidden md:table-cell">Duration</th>
                    <th className="w-[20%] hidden lg:table-cell">
                      Casting Time
                    </th>
                  </tr>
                ...
                <tbody>
                  {spellList.Spells.filter(
                    ...
                    >
                      ...
                      <td>{spell.range}</td>
                      <td className="hidden md:table-cell">{spell.duration}</td>
                      <td className="hidden lg:table-cell">
                        {spell.castingTime}
                      </td>
                    </tr>
      ...
      <CommentSection id={spellList.id} model={AssociatedModel.SPELL_LIST} />
    </main>
  );
};
