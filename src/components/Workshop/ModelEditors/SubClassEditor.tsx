'use client';

import SidebarMetaSelector from '@/components/Dashboard/SidebarMetaSelector';
import FormField from '@/components/UI/Formik/FormField';
import LoadingButton from '@/components/UI/Formik/LoadingButton';
import Loading from '@/components/UI/Loading';
import useMetadata from '@/hooks/useMetadata';
import useWorkshopEditor from '@/hooks/useWorkshopEditor';
import { SubclassEditorData } from '@/lib/types/workshop';
import { memoizeGetClass } from '@/Utility/Indexed/globalCache';
import { Form, Formik } from 'formik';
import { ChangeEvent, useState } from 'react';
import * as Yup from 'yup';

const SubclassEditor = () => {
  const { metadata: classes, loading } = useMetadata('class');
  const [show, setShow] = useState(false);

  const { data, updateData } = useWorkshopEditor<SubclassEditorData>({
    flavorText: '',
    description: '',
    classData: null,
  });

  const featSchema = Yup.object().shape({
    flavorText: Yup.string()
      .required('Required')
      .max(150, 'Max 150 characters'),
    description: Yup.string().required('Required'),
  });

  if (!data) return <Loading />;

  return (
    <div>
      <Formik
        initialValues={data}
        validationSchema={featSchema}
        onSubmit={(values) => console.log(values)}
      >
        <Form>
          <FormField
            as="textarea"
            name="flavorText"
            label="Flavor Text - Briefly describe the subclass - Max 150 characters"
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) =>
              updateData({
                ...data,
                flavorText: e.target.value,
              })
            }
            formProps={{
              value: data.flavorText,

              placeholder:
                'Ex. You have a knack for learning new things, picking up new skills and abilities quickly.',
            }}
          />
          <FormField
            as="textarea"
            name="description"
            label="Description - Describe any preliminary information about the subclass before the mechanics are explained in the features section."
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) =>
              updateData({
                ...data,
                description: e.target.value,
              })
            }
            formProps={{
              value: data.description,
              placeholder:
                'Ex. Years of study have given you a wealth of knowledge. You gain the following benefits:',
            }}
          />
          <LoadingButton
            className="btn btn-primary"
            isLoading={loading}
            onClick={() => setShow(true)}
          >
            Choose Class
          </LoadingButton>
          <SidebarMetaSelector
            model="class"
            metadata={classes}
            loading={loading}
            show={show}
            setSelected={async (selected) => {
              setShow(false);
              if (!selected) return;
              const classData = await memoizeGetClass({
                query: selected.id,
                type: 'id',
              });
              updateData({
                ...data,
                classData: {
                  id: selected.id,
                  name: selected.name,
                  subClassLevels: classData?.subClassFeatureLevels || [],
                },
              });
            }}
          />
        </Form>
      </Formik>
    </div>
  );
};

export default SubclassEditor;
