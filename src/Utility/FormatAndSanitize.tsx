...
const P = ({
  ...
  useEffect(() => {
    ...
    const sanitizedDescription = str as string;
    ...
    function findTerms(str: string): Term[] {
      const pattern = termDictionary.map((term) => term.term).join('|');
      ...
      curlyMatches.forEach((match) => {
        const emptyString = ' '.repeat(match[1]);
        noCurlyString =
