/*
  Warnings:

  - Made the column `userId` on table `WorkshopItem` required. This step will fail if there are existing NULL values in that column.

*/
-- DropForeignKey
ALTER TABLE "WorkshopItem" DROP CONSTRAINT "WorkshopItem_userId_fkey";

-- AlterTable
ALTER TABLE "WorkshopItem" ALTER COLUMN "userId" SET NOT NULL;

-- AddForeignKey
ALTER TABLE "WorkshopItem" ADD CONSTRAINT "WorkshopItem_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
